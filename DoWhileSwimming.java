...
public class DoWhileSwimming
{
    ...
    //at 80.5F Galland and Gofus swim for the same time
    //Gallant doesnt swim, Gofus stops after 1min
    //Gallant checks the min temp before entering
    //Gofus dives in, swims a min then realises its too cold and stops
    //a while loop checks the condition before running the body, which is why Gallant doesnt enter.
    //a do loop runs the body once then checks the condition, which is why Gofus dives in before realising its too cold.
    //while is pre-test loop, do-while is a post-test loop
